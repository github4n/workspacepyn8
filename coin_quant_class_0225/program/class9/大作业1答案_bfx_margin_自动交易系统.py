...
# 在运行程序之前需将ccxt升级至最新版本，并在margin账户内存入美元
...
from program.class9.Trade import next_run_time
...
# =====初设参数设定
# 基本参数
time_interval = '15m'  # 间隔运行时间，15m代表15分钟，可以改为5m,10m,30m等。注意，不能超过1h
para = [100, 3]  # 策略参数
leverage = 0.1  # 杠杆倍数，范围(0, 3]。bfx最多支持3.33倍杠杆，但实际当中建议不要超过3。0.5倍杠杆，代表使用50%的保证金进行下单
symbol = 'xrp/usd'
ccxt_symbol = 'XRP/USD'
...
def real_time_signal_bolling(df, para=[100, 2]):
    ...
    signal = None
    # 找出做多信号
    if (df.iloc[-1]['close'] > upper) and (df.iloc[-2]['close'] <= upper2):
        signal = 1
    # 找出做空信号
    elif (df.iloc[-1]['close'] < lower) and (df.iloc[-2]['close'] >= lower2):
        signal = -1
    # 找出做多平仓信号
    elif (df.iloc[-1]['close'] < median) and (df.iloc[-2]['close'] >= median2):
        signal = 0
    # 找出做空平仓信号
    elif (df.iloc[-1]['close'] > median) and (df.iloc[-2]['close'] <= median2):
        signal = 0
    ...
    for i in range(5):
        try:
            ...
            # 市价单
                ...
                condition1 = position_info['symbol'] == trade_coin + base_coin.lower()
                position_id = list(position_info.loc[condition1, 'id'])[0]

                order_info = exchange.private_post_position_close(params={'position_id': int(position_id)})
                print('关仓成功')
            else:
                pass
...
def fetch_account_info(exchange, base_coin, trade_symbol):
    ...
    while True:
        ...
        try:
            ...
            print(e)
            continue
        data = pd.DataFrame(data)  # 将数据转化为df格式

        data.rename(columns={0: '交易账户', 1: '币种', 2: '数量', 3: 'unknow', 4: 'unknow2'}, inplace=True)
        ...
        if len(position_info) > 0:  # 当持仓信息信息不为空时
            position_info = pd.DataFrame(position_info)  # 将数据转化为df格式
            position_info.rename(columns={0: '交易对', 1: '状态', 2: '持仓量', 3: '成本价格', 4: '借币利息', 5: 'unknow1', 6: '利润', 7: '损益比例', 8: '爆仓价格',
                         9: 'unknow3'}, inplace=True)
...
def get_bitfinex_candle_data(exchange, symbol, time_interval, limit):
    ...
    # 如果在服务器时间为格林威治时间的服务器上运行，只需将加号连同后面的东西一起注释就可以了
...
def main():
    ...
    while True:
        ...
        if count > 10:
            send_dingding_msg('本周期获取不到k线跳入下个周期')
            main()
        if _temp.empty:
    ...
    if account_info['仓位数量'] < 0 and signal == 0:
        ...
        print('\n关闭空单')
        place_order(exchange=exchange, order_type='close', buy_or_sell='buy', price=now_price * 1.02,
                    symbol=ccxt_symbol,
                    amount=trade_amount)
        msg_content += '\n_关闭空单_'
    ...
    # 之前为多单，现在要平仓的情况
        ...
        trade_amount = abs(account_info['仓位数量'])
        print('\n关闭多单')
        place_order(exchange=exchange, order_type='close', buy_or_sell='sell', price=now_price * 0.98,
                    symbol=ccxt_symbol,
                    amount=trade_amount)
        msg_content += '\n_关闭多单_'
        msg_content += '\n交易数量:' + str(trade_amount) + '\n'
    ...
    # 之前没有仓位，现在要开多单的情况
        ...
        place_order(exchange=exchange, order_type='limit', buy_or_sell='buy', price=now_price * 1.02,
                    symbol=ccxt_symbol,
                    amount=trade_amount)
        msg_content += '\n_开多单_'
        msg_content += '\n交易数量:' + str(trade_amount) + '\n'
    ...
    # 之前空仓 现在要开空单的情况
        ...
        print('\n开空单')
        trade_amount = abs(account_info['账户保证金'] * leverage / now_price)  # 计算开仓量
        place_order(exchange=exchange, order_type='limit', buy_or_sell='sell', price=now_price * 0.98,
                    symbol=ccxt_symbol,
                    amount=trade_amount)
        msg_content += '\n_开空单_'
        msg_content += '\n交易数量:' + str(trade_amount) + '\n'
