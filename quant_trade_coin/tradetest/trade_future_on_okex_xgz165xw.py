...
import sys
sys.path.append('/quant')  #  本地连接服务器加搜索路劲，调试用
...
while True:
    ...
    try:
        ...
        #  冰山委托不能及时成交的问题,无持仓，开仓的情况，先取消挂单，再开仓
        ...
        if condition2:
            ...
            if short_qty > 0:
                ...
                order_info = fuc.futures_post_order_algo(exchange, symbol, action_type, size)  # 下单平空
                # 邮件
                email_title += '平空'
            ...
            if condition1 & (long_qty == 0):
                ...
                order_info = fuc.futures_post_order_algo(exchange, symbol, action_type, size)  # 下单开多
                # 邮件
                email_title += '开多'
                email_content += '数量：' + str(size) + '\n'
                fuc.auto_send_email('444073364@qq.com', email_title, email_content)
            ...
            if long_qty > 0:
                ...
                order_info = fuc.futures_post_order_algo(exchange, symbol, action_type, size)  # 下单平多
                # 邮件
                email_title += '平多'
                email_content += '数量：' + str(size) + '\n'
                fuc.auto_send_email('444073364@qq.com', email_title, email_content)
            if condition4 & (short_qty == 0):
                ...
                order_info = fuc.futures_post_order_algo(exchange, symbol, action_type, size)  # 下单开空
                # 邮件
                email_title += '开空'
                email_content += '数量：' + str(size) + '\n'
                fuc.auto_send_email('444073364@qq.com', email_title, email_content)
