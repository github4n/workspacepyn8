...
# =====导入数据
df = pd.read_hdf('F:\workspacepyn\coin_quant_class_190806\data\class8\eth_1min_data.h5', key='all_data')

# df = df[df['candle_begin_time']>="2018/03/01 00:00:00"]
# df = df[df['candle_begin_time']<="2018/03/20 10:56:00"]
# print(df)
# exit()
# =====转换为其他分钟数据
rule_type = '15T'
...
# ===合并做多做空信号，去除重复信号
#极端情况同时出现开多信号和空头平仓信号，同时出现开空信号和多头平仓信号
# print(df)
df['signal'] = df[['signal_long', 'signal_short']].sum(axis=1, skipna=True,min_count=1)
# print(df[df['signal_long']==1][df['signal_short']==0 ])
# print(df)
# exit()
...
# df = df[df['candle_begin_time']<="2018/03/01 15:00:00"]
